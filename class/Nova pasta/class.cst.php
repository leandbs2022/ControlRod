	...
	class cst
		...
		function logC($senha,$nome)
				{
			    $query = mysqlii_query( "SELECT * FROM `login` WHERE Nome='$nome'" );
	    		if ( mysqlii_num_rows( $query ) ) {
	      			while ( $array = mysqlii_fetch_row( $query ) ) {
	        			$cript = $array[ 4 ];
		...
		function usuarios($cod,$nome,$senha,$per,$dep)
				{
					...
					if(mysqlii_num_rows($query))
						{
						$query = mysqlii_query("UPDATE `login` SET `Nome`='$nome',`Senha`='$senha',`Nivel`='$per',`dep`='$dep' WHERE nome='$nome'") or die(mysqlii_error());
						...
						{
							$cod = $cod + 1 ;
							$query = mysqlii_query("INSERT INTO login (Codigo, Nome, Senha, Nivel,dep) VALUES ('$cod', '$nome', '$senha', '$per','$dep')") or die(mysqlii_error());
	...
	function seg($equip,$area,$dica,$usuario,$senha,$infor)
			{
				...
				$query = mysqlii_query("SELECT * FROM `infor_secret` WHERE equip_prod='$equip'");
				if(mysqlii_num_rows($query))
						{
	$query = mysqlii_query("UPDATE `infor_secret` SET `area`='$area',`dica`='$dica',`usuario`='$usuario',`senha`='$senha' ,`infor_`='$infor' WHERE equip_prod='$equip'") or die(mysqlii_error());
							echo "<H4>Essas informações ja existe e seus dados foram atualizados!!!</H4>";
							return $query;	
						}
						else
						{
							$query = mysqlii_query("INSERT INTO infor_secret (equip_prod,area,dica,usuario,senha,infor_) VALUES ('$equip', '$area', '$dica','$usuario','$senha' ,'$infor')") or die(mysqlii_error());
...
function adicionar($cnpj, $ip, $pc, $dep, $resp, $team, $ramal,$carg,$loc,$desc) 
			{
				...
				$data = date('Y-m-d H:i');
					...
					if(empty($cnpj) || empty($ip)|| empty($pc)|| empty($dep)|| empty($resp)|| empty($team)|| empty($ramal)|| empty($carg)|| empty($loc)|| empty($desc))
						...
						{
$resultado = mysqlii_query("SELECT * FROM `red_tel` WHERE ip='$ip'");
if(mysqlii_num_rows($resultado)){	
		
		
		$query = mysqlii_query("UPDATE `red_tel` SET 					`cnpj`='$cnpj',`ip`='$ip',`host`='$pc',`dep`='$dep',`resp`='$resp',`team`='$team',`ram`='$ramal',`carg`='$carg',`loc`='$loc',`observar`='$desc' WHERE ip='$ip'") or die(mysqlii_error());
							...
							{
$query = mysqlii_query("INSERT INTO `red_tel`(`cnpj`, `ip`, `host`, `dep`, `resp`, `team`, `ram`, `carg`, `loc`, `observar`) VALUES ('$cnpj', '$ip', '$pc', '$dep', '$resp', '$team','$ramal','$carg','$loc','$desc')") or die(mysqlii_error());
		echo "Operacao foi concluida com sucesso!!!";
			...
			function adicionar_dep($dep) 
			{
				...
				if(empty($dep))
						...
						{
							$resultado = mysqlii_query("SELECT * FROM `depart` WHERE dep='$dep'");
							if(mysqlii_num_rows($resultado))
								{	
								$query = mysqlii_query("UPDATE `depart` SET `dep`='$dep' WHERE dep='$dep'") or die(mysqlii_error());
								echo "<h4>O cadastro foi alterado com sucesso!!!</h4>";	
								return $query;
								}
								else
									{
									$query = mysqlii_query("INSERT INTO `depart`(`dep`) VALUES ('$dep')") or die(mysqlii_error());
			...
			function adicionar_impr($ip,$marca,$modelo,$local) 
				...
				if(empty($ip) || empty($marca) || empty($modelo) || empty($local) )
						...
						{
							$resultado = mysqlii_query("SELECT * FROM impr WHERE ip = '$ip'");
							if(mysqlii_num_rows($resultado))
								{	
							$query = mysqlii_query("UPDATE `impr` SET `ip`='$ip',`loc`='$local',`marc`='$marca',`mode`='$modelo' WHERE ip='$ip'") or die(mysqlii_error());
								echo "<h4>O cadastro foi alterado com sucesso!!!</h4>";	
								return $query;
								}
								else
									{
									$query = mysqlii_query("INSERT INTO `impr`(`ip`,`marc`,`mode`,`loc`) VALUES ('$ip','$marca','$modelo','$local')") or die(mysqlii_error());
									echo "<h4>Operacao foi concluida com sucesso!!!</h4>";
									return $query;
									}
						}
		...
		function delete($ip)
			{	
					...
					if(mysqlii_num_rows($resultado))
						{
							$query = mysqlii_query("DELETE FROM `red_tel` WHERE ip='$ip'");
			...
			{	
					...
					if(mysqlii_num_rows($resultado))
						{
							$query = mysqlii_query("DELETE FROM `impr` WHERE ip='$ip'");
							echo "<h4>Registro deletado!!!</h4>";
			...
			$resultado = mysqlii_query("SELECT * FROM `tel_sen` WHERE senha='$senha'");
			if(mysqlii_num_rows($resultado))
					{	
		$query = mysqlii_query("UPDATE `tel_sen` SET `Nome`='$nome' WHERE Senha='$senha'") or die(mysqlii_error());
										echo "<h4>O cadastro foi alterado com sucesso!!!</h4>";
							...
							{
$query = mysqlii_query("INSERT INTO `tel_sen`(`Nome`, `Senha`) VALUES ('$nome', '$senha')") or die(mysqlii_error());
		echo "<h4>Operacao foi concluida com sucesso!!!</h4>";
								return $query;
							}
		...
		function adicionar_secret($prod,$area,$dica,$usu,$sen,$desc)
			...
			if(empty($prod) || empty($area)|| empty($dica)|| empty($usu)|| empty($sen)|| empty($desc))
						...
						{
$resultado = mysqlii_query("SELECT * FROM `infor_secret` WHERE equip_prod='$prod'");
if(mysqlii_num_rows($resultado)){	
		
		
		$query = mysqlii_query("UPDATE `infor_secret` SET `equip_prod`='$prod',`area`='$area',`dica`='$dica',`usuario`='$usu',`senha`='$sen',`infor_imp`='$desc' WHERE euip_prod='$prod'") or die(mysqlii_error());
										echo "<h4>O cadastro foi alterado com sucesso!!!</h4>";
...
$query = mysqlii_query("INSERT INTO `red_tel`(`cnpj`, `ip`, `host`, `dep`, `resp`, `team`, `ram`, `carg`, `loc`, `observar`) VALUES ('$cnpj', '$ip', '$pc', '$dep', '$resp', '$team','$ramal','$carg','$loc','$desc')") or die(mysqlii_error());
		...
		function lista_dados($item)
			{
				$query = mysqlii_query("SELECT $item FROM `red_tel` WHERE  1");
				while($monta = mysqlii_fetch_assoc($query))
	  				{
			...
			if(empty($campo))
				{
					$query = mysqlii_query("SELECT * FROM  `red_tel` WHERE ram <> '0' order by host asc");		
				}else{
					if ($nivel == 2){
						$tipo_t == "Re";
						$query = mysqlii_query("SELECT * FROM `red_tel` WHERE  $campo like '$busca%' order by host asc");
						}else{
						$tipo_t == "R";
						$query = mysqlii_query("SELECT * FROM `red_tel` WHERE  $campo like '$busca%' and ram <> '0' order by host asc");
						}
				}
		...
		function lista($campo,$busca,$nivel,$tipo_t)
			...
			if(empty($campo))
				...
				{
					if($nivel <> 2){
						$query = mysqlii_query("SELECT * FROM `red_tel` WHERE  $campo like'$busca%' and ram <> '0' order by carg asc");
					}else{
						if($tipo_t == "R"){
							     $query = mysqlii_query("SELECT * FROM `red_tel` WHERE  $campo like'$busca%' and ram <> '0' order by carg asc");
							}else{
							     $query = mysqlii_query("SELECT * FROM `red_tel` WHERE  $campo like'$busca%'  order by carg asc");
						}
					}
				}
	...
	function apar_wke($busca)
	{
		...
		$tipoL = "CLINICA";
		$tip = ""; 
		$query = mysqlii_query("SELECT ip,host,loc FROM `red_tel` WHERE  host like '$busca%' or host like '$b%' or host like '$c%' or host like '$d%' order by loc asc");
				if(mysqlii_num_rows($query))
				{
					$estilos[] = "text-align: center; background-color: rgb(128, 128, 128);font-size:13px";
					$estilos[] = "text-align: center;font-size:13px";
					...
					while($array = mysqlii_fetch_row($query)) 
						{
						$tip = $array[2];
						if($tip == "CLINICA"){$vsl= "L2 SUL";}
						if($tip == "CLINICA TAG"){$vsl= "TAGUATINGA";}
	...
	function lista_docs($entrada,$saida)
		{	
				$query = mysqlii_query("SELECT * FROM movi_c WHERE dt_sai >= '$entrada' and dt_sai <= '$saida' order by dt_sai asc");
				if(mysqlii_num_rows($query))
				{
					$estilos[] = "text-align: center; background-color: rgb(128, 128, 128);font-size:13px";
					$estilos[] = "text-align: center;font-size:13px";
		...
		$query = mysqlii_query("SELECT * FROM  `impr` WHERE 1 order by loc asc");		
				if(mysqlii_num_rows($query))
				{
					$total = 0;
					$estilos[] = "text-align: center; background-color: rgb(128,128,128);font-size:13px";
		...
		$query = mysqlii_query("SELECT * FROM `infor_secret` WHERE  equip_prod='$busca' ");
					if(mysqlii_num_rows($query))
						{
							while($array = mysqlii_fetch_row($query)) 
								{		
		...
		function delete_seg($equip){
			...
			$resultado = mysqlii_query("SELECT * FROM `infor_secret` WHERE equip_prod='$equip'");
			if(mysqlii_num_rows($resultado))
				{
					$query = mysqlii_query("DELETE FROM `infor_secret` WHERE equip_prod='$equip'");
        ...
        function imprimir($campo,$busca,$nivel)
		{
				...
				{
					$query = mysqlii_query("SELECT * FROM `impr` WHERE  $campo='$busca' order by loc asc");
				}
