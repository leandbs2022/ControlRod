...
function adicionar($produto,$marca,$modelo,$quant,$datae,$tipo,$infor){
    if ( empty($produto) || empty($marca) || empty($modelo) || empty($quant) || empty($datae)) {
        ...
        $resultado = mysql_query( "SELECT * FROM `equip` WHERE `produto`='$produto'");
        if ( mysql_num_rows($resultado ) ) {
            $query = mysql_query( "UPDATE `equip` SET `produto`=$produto,`marca`=$marca,`modelo`=$modelo,`quant`=$quant,`data_ent`=$datae,`tipo`= $tipo,`Infor_Adic`=$infor WHERE produto='$produto'");
            echo "Alterado com sucesso";
          } else {
            $query = mysql_query( "INSERT INTO `equip`(`produto`,`marca`,`modelo`,`quant`,`data_ent`,`tipo`,`Infor_Adic`) VALUES ('$produto','$marca','$modelo','$quant','$datae','$tipo','$infor')" );
...
function visualizar($loc,$id){
    if($loc == 1){
        $resultado = mysql_query("SELECT * FROM `equip` WHERE id='$id'");

    }else{
        $resultado = mysql_query( "SELECT * FROM `equip` WHERE 1 ");
    }
    if ( mysql_num_rows( $resultado) ) {
