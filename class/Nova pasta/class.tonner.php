...
class tonner {
  function visualizar_media() {
    ...
    //ConexãO AO bd
    $query = mysqlii_query( "SELECT ton FROM `impr_tonner` WHERE 1" );
    if ( mysqlii_num_rows( $query ) ) {
      while ( $array = mysqlii_fetch_row( $query ) ) {
        $ton = $array[ 0 ];
        $sql = mysqlii_query( "SELECT * FROM `media_ton` WHERE modelo='$ton' and data_sai between '$dat' and '$today'" )or die( mysqlii_error() );
        if ( mysqlii_num_rows( $sql ) ) {
          while ( $model = mysqlii_fetch_row( $sql ) ) {
            $total = $total + $model[ 3 ];
            if ( $total != 0 ) {
  ...
  function adicionar_media( $ton, $today, $unid, $quant ) {
    ...
    $query = mysqlii_query( "INSERT INTO media_ton (modelo, data_sai, loc, quant) VALUES ('$ton', '$today', '$unid', '$quant')" )or die( mysqlii_error() );
  }
  ...
  function adicionar_tonner( $ton, $model, $quant ) {
    ...
    if ( empty( $ton ) || empty( $model ) ) {
      ...
      $resultado = mysqlii_query( "SELECT * FROM impr_tonner WHERE ton='$ton'" );
      if ( mysqlii_num_rows( $resultado ) ) {
        $query = mysqlii_query( "UPDATE `impr_tonner` SET `ton`='$ton',`model`='$model',`quant`='$quant' WHERE ton='$ton'" )or die( mysqlii_error() );
        echo "<h4>O cadastro foi alterado com sucesso!!!</h4>";

      } else {
        $query = mysqlii_query( "INSERT INTO `impr_tonner`(`ton`,`model`,`quant`) VALUES ('$ton','$model','$quant')" )or die( mysqlii_error() );
...

  {
    if ( empty( $ton ) ) {
      ...
      if ( mysqlii_num_rows( $resultado ) ) {
        ...
        $query = mysqlii_query( "DELETE FROM `impr_tonner` WHERE ton='$ton'" );
  ...
  function lista_tonner( $busca ) {
    if ( empty( $busca ) ) {
      $query = mysqlii_query( "SELECT * FROM  `impr_tonner` WHERE 1 order by model asc" );
    } else {
      $query = mysqlii_query( "SELECT * FROM `impr_tonner` WHERE  modelo='$busca' order by model asc" );
    }
    if ( mysqlii_num_rows( $query ) ) {
      $total = 0;
